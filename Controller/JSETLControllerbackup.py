...
class JSETLController(ConfigureDelegate):
    ...
    def findkeywords(self, configuremodel):
        print("开始找寻表头最下层title")
            ...
            try:
                ...
                for sheetname in sheetnames:
                    ...
                    if len(mergecells) == 0:
                        ...
                        # print(lowestnodes)
                        rlow, rhign, clow, chigh = lowestnodes[0].cellrange
                        headrow = rhign + 1
    ...
    # 内部用的,用于快速生成模板表头
    def CheckSheetHead(self, sourcemodelpath, standardpath, startrow):
        print("查找标准模板的部分")
        ...
        for path in exceldatalist:
            try:
                ...
                for sheetname in sheetnames:
                    ...
                    for index, cell in enumerate(resultCells):
                        crlow, crhign, cclow, cchigh = cell
                        if crlow <= rangerow and crlow > startrow:
                            mergecells.append(cell)
                    # 默认第一行为表头 没有合并单元格的情况
                    if len(mergecells) == 0:
                        headrow = 1
                        ...
                        headrow = rhign + 1
                    # 找到每个 sheet 的表头范围进行和之前的对比 TODO 这里可以对提取出来的表头进行处理
                    curdf = pd.read_excel(path, sheetname,skiprows = startrow, header=None)
                    # 每个进行遍历对比,完全不同则添加到 list 中
                    ...
                    if flag:
                        ...
                        filename, suffix = JSExcelHandler.SplitPathReturnNameAndSuffix(path)
                        # 这里要保存为 xlsx 为了兼容合并单元格的功能
                        objectpath = "{}/{}_{}.{}".format(standardpath, filename, sheetname, 'xlsx')
                        print("{}.xlsx:{}".format(filename, startrow))
                        copyfile(path, objectpath)
    ...
    # 按照 这里是完全的还原表格表头, mergecell 的范围重写入表头并按原表合并单元格
...
if __name__ == '__main__':
    ...
    # 快速生成模板文件
    # controller = JSETLController()
    # controller.CheckSheetHead(r'/Users/sun/Desktop/AUTOEtlTest/heads',r'/Users/sun/Desktop/AUTOEtlTest/standard', 1)
    ...
    # 合并去重测试
    # JSExcelHandler.appendData(r'/Users/sun/Desktop/AUTOEtlTest/result',r'/Users/sun/Desktop/AUTOEtlTest/standard')

    # num_str1 = '34052419800101001X'
    # num_str2 = '340524198001010011'
    # JSExcelHandler.checkIDNumber(num_str1)
    # JSExcelHandler.checkIDNumber(num_str2)
    def score_verification(row, *argsss):
        for foo in argsss:
            if foo == '身份证':
            ...
            # else:
            #     # 其余的进行空值的判断
            #     if not JSExcelHandler.checkIsNull(row[foo]):
            #         print(f'{row}')

    df = pd.read_excel(r'/Users/sun/Desktop/AUTOEtlTest/data/test.xls')
    keywords = ['身份证', '单位']
    list = list(keywords)
    df['身份证校验'] = df['身份证'].apply(score_verification, args=list)

    print(df)
    # df.apply(score_verification, axis=1, args=list)